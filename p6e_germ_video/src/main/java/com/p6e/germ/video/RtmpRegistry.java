...
import com.p6e.germ.video.message.RtmpDefaultMessage;
 ...
 * @version 1.0
 */
public final class RtmpRegistry {
    ...
    /** 消息核心 */
    private static final Map<String, RtmpMessage> MESSAGE_CORE = new HashMap<>();

    /** 处理核心 */
    private static final Map<String, RtmpActuator> ACTUATOR_CORE = new HashMap<>();
    ...
    static {
        /* 设置默认的消息核心 */
        MESSAGE_CORE.put(DEFAULT_NAME, new RtmpDefaultMessage());
        /* 设置默认的处理器核心 */
        ACTUATOR_CORE.put(DEFAULT_NAME, new RtmpDefaultActuator());
    }
     ...
     * @return 获取消息的核心
     */
    public static RtmpMessage getMessageCore(int key) {
        RtmpMessage message = MESSAGE_CORE.get(String.valueOf(key));
        if (message == null) {
     ...
     * @return 消息的处理器
     */
    public static RtmpActuator getActuatorCore(int key) {
        RtmpActuator actuator = ACTUATOR_CORE.get(String.valueOf(key));
        if (actuator == null) {
    ...
    public synchronized static void registerMessage(int key, RtmpMessage value) {
        MESSAGE_CORE.put(String.valueOf(key), value);
    }

    public synchronized static void registerActuator(int key, RtmpActuator value) {
